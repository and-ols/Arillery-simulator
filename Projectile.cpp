...
void Projectile::advance(double time)
{
   ...
   double acceleration = velocity.getAcceleration(dragForce, MASS);
   double gravity = lookUpGravity();
   double angle = Direction().getAngleFromComponents
      (velocity.getDX(), velocity.getDY());
   updatePosition(acceleration, gravity, angle, time);
   flightTime += time;
}
...
void Projectile::draw(list<Position> projectilePath)
{
   ...
   int itr = 0;
   for (Position position : projectilePath)
      gout.drawProjectile(position, 0.5 * (double)itr++);
}
...
double Projectile::lookUpMach()
{
   ...
   return velocity.getSpeed() / speed_of_sound;
}
...
void Projectile::setDirection(double angle)
{
   velocity.setDX(velocity.computeHorizontalComponent(angle, INITIALVELOCITY));
   velocity.setDY(velocity.computeVerticalComponent(angle, INITIALVELOCITY));
}

/*************************************************************************
* SET FLYING
* Changes flying status. Then if it is now flying, apply inital velocity.
*************************************************************************/
void Projectile::setFlying(bool newFlyingStatus)
{
   isFlying = newFlyingStatus;
   //if (isFlying)
   //   this->velocity = Velocity(INITIALVELOCITY, direction.getRadians());
}
...
void Projectile::updatePosition(double acceleration, 
                 double gravity, double angle, double time)
{
   // Calculate acceleration.
   double ddx = velocity.computeHorizontalComponent(angle, 
                                             acceleration);
   double ddy = velocity.computeVerticalComponent(angle, 
                                    acceleration) - gravity;

   // Save the initial position for recording distance.
   double beforeMovingX = position.getMetersX();
   ...
   if (beforeMovingX > position.getMetersX())
      flightDistance += beforeMovingX - position.getMetersX();
   else
      flightDistance += position.getMetersX() - beforeMovingX;

   // Lastly, update the velocity.
   velocity.addAccelerationToVelocity(ddx, ddy, time);
}
...
double Projectile::dragForceEquation(double dragCo, double density)
{
   return 0.5 * dragCo * density *
      (velocity.getSpeed() * velocity.getSpeed()) * SURFACEAREA;
}